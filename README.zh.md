...
其**以 artifact 为驱动**的设计消除了长上下文依赖，避免了冗余的 token 消耗。

QUIT 同时完整支持**端到端模式**——只需设定研究主题，QUIT即可无干预地自动运行至完成。
...
所有协调通过 artifact 文件完成——Agent 之间不共享跨调用的会话记忆。这使整个过程**可追溯、可复现、可恢复**。

> 🙋 **人机协同控制点：** 研究者可在任意状态后暂停，检查artifact、编辑中间文件（EvidenceCard、BuildSpec、生成代码、实验结果），并从选定状态继续运行。
...
- 👁️ 在 **Artifacts** 面板中**查看**制品（EvidenceCard、BuildSpec、代码、结果、论文）
